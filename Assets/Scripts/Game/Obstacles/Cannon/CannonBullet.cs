...
public class CannonBullet : MonoBehaviour {
    ...
    public IEnumerator Shoot(Vector2Int init, Vector2Int target)
    {
        ...
        if (ObstacleMap.ObstacleIn(target) != null && ObstacleMap.ObstacleIn(target).GetComponent<ICannonBulletTarget>() != null)
        {
            yield return ObstacleMap.ObstacleIn(target).GetComponent<ICannonBulletTarget>().OnCannonBulletTouch();
        }
